...
describe('interview test', () => {
  it('beceom-a-customer form test', () => {
    ...
    cy.get("[data-qa='billing_city']").type("Houston")

    // cy.get("[aria-labelledby='foundation-text-input-1840']").click()
   
     //cy.get("[value='TX']").first().click()

    // cy.get("[aria-labelledby='foundation-text-input-1842']").click()
   ...
   // cy.get("[aria-labelledby='foundation-text-input-2190']").should("not.be.visible")
     ...
     cy.get("[data-qa='shipping_address']").should('have.value','1390 Enclave Parkway')

     cy.get("[data-qa='shipping_city']").should('have.value','Houston')
